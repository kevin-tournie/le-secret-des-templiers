    ...
    <div class="bg-white shadow-lg rounded-lg p-6 w-full max-w-md">
      ...
      <UForm :schema="createTeamSchema" :state="state" class="flex flex-col mb-4 gap-5" @submit="onSubmit">
        <UFormGroup label="Nom d'équipe" name="teamName">
          <UInput v-model="state.teamName" size="sm"
        ...
        <UFormGroup label="Nom de l'entreprise (Facultatif)" name="companyName">
          <UInput v-model="state.companyName" size="sm"
            class="w-full border-gray-300 focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50 rounded-md shadow-sm">
          </UInput>
        </UFormGroup>
        <UButton type="submit" class="w-full text-white font-bold py-2 px-4 rounded text-center">Créer une équipe
        </UButton>
      </UForm>
    </div>
  </div>
